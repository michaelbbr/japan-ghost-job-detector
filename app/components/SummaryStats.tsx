...
export const SummaryStats: React.FC<SummaryStatsProps> = ({ summary, onFilterClick, lang = 'zh' }) => {
    ...
    <div className="bg-white rounded-2xl border border-slate-200/80 shadow-sm p-5 sm:p-6 mb-6">
      <div className="flex flex-wrap items-center justify-between gap-4 pb-4 border-b border-slate-100 mb-5">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">
            {t.summarySubtitle(summary.total)}
          </p>
        </div>
        <div className="flex items-center gap-3">
          <div className="text-right">
            <div className="text-xs font-semibold uppercase tracking-wider text-slate-400">{t.avgScoreLabel}</div>
          ...
          <div className={`px-3 py-2 rounded-xl border font-bold text-xs ${getScoreColor(summary.avgScore)}`}>
            {summary.avgScore >= 70 ? t.overallHigh : summary.avgScore >= 45 ? t.overallMed : t.overallLow}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 sm:gap-4">
        ...
        >
          ...
          <div className="text-[11px] text-red-600/80 mt-1">{t.statHighRiskDesc}</div>
        </button>
        ...
        >
          ...
          <div className="text-[11px] text-amber-700/80 mt-1">{t.statSuspiciousDesc}</div>
        </button>
        ...
        >
          ...
          <div className="text-[11px] text-emerald-700/80 mt-1">{t.statSafeDesc}</div>
        </button>
        ...
        >
          ...
          <div className="text-[11px] text-purple-600/80 mt-1">{t.statMinashiDesc}</div>
        </button>
        ...
        >
          ...
          <div className="text-[11px] text-rose-600/80 mt-1">{t.statScamDesc}</div>
        </button>
        ...
        >
          <div className="text-xs font-medium text-blue-700 flex items-center justify-between">
            ...
            <span className="text-[10px] bg-blue-200/60 px-1.5 py-0.5 rounded text-blue-800">重複</span>
          </div>
          ...
          <div className="text-[11px] text-blue-600/80 mt-1">{t.statDuplicatesDesc}</div>
        </button>
      </div>
    </div>
  );
